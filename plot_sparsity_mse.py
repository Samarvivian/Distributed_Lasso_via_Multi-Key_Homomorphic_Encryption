#!/usr/bin/env python3
"""
Plot MSE convergence for TRIAD at different sparsity levels.

Usage:
    cd solve && python plot_sparsity_mse.py
"""
...
# Adaptive_TRIAD_log.csv has no mse column, so only 20/30/40 here
SPARSITY_SERIES = [
...
def find_divergence(arr, explosion_threshold=1e4, jump_factor=100, jump_min=100):
    prev = None
...
def plot_series(ax, iters, arr, color, marker, lw, ms, label, zorder=2):
    div_idx = find_divergence(arr)
    ...
    valid_arr   = arr[:div_idx]
    ax.plot(valid_iters, valid_arr,
            color=color, linewidth=lw,
            marker=marker, markevery=MARKER_INTERVAL,
            markersize=ms, label=label, zorder=zorder)
    ax.scatter(valid_iters[-1], valid_arr[-1],
               color=color, marker='X', s=140,
               edgecolors='black', linewidths=0.8,
               zorder=zorder + 5)
    return (valid_iters[-1], valid_arr[-1], div_idx)
...
def main():
    ...
    for fname, label, color, marker in SPARSITY_SERIES:
        ...
        if 'mse' not in df.columns:
            print(f'  [WARN] {fname} has no mse column, skipping.')
            continue
        iters = df['iter'].values
        mse   = parse_col(df['mse'])
        res   = plot_series(ax, iters, mse, color, marker,
                            lw=2.0, ms=6, label=label, zorder=2)
        if res is not None:
            x, y, di = res
            divergence_annotations.append(
                (x, y, f'iter {di}, diverged', color))
        fin = mse[~np.isnan(mse)]
        last_str = f'{fin[-1]:.6f}' if len(fin) > 0 else 'N/A'
        print(f'  {label}: {len(iters)} iters, final MSE={last_str}')
    ...
    ax.set_ylabel('MSE (log scale)', fontsize=14, fontweight='normal')
    ...
    for fname, _, _, _ in SPARSITY_SERIES:
        df = load_csv(fname)
        if df is None or 'mse' not in (df.columns if df is not None else []):
            continue
        mse = parse_col(df['mse'])
        div = find_divergence(mse)
        valid = mse[:div] if div is not None else mse
    ...
    for fmt in ['pdf', 'png']:
        path = os.path.join(out_dir, f'sparsity_mse.{fmt}')
