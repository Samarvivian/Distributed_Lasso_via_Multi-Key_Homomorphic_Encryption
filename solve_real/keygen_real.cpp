/**
 * @file keygen_real.cpp
 * @brief TRIAD Trusted Dealer for Riboflavin real-data experiment
 *
 * N_FEAT=4088 requires BatchSize >= 4096.
 * We use BatchSize=8192 for safety margin.
 *
 * Usage:
 *   ./keygen_real
 * Outputs: keys/ directory with all threshold CKKS keys.
 */
...
static const int MULT_DEPTH       = 15;
static const int SCALING_MOD_SIZE = 40;
static const int FIRST_MOD_SIZE   = 45;
static const int BATCH_SIZE       = 8192;   // N_FEAT=4088, must have BatchSize>=4096
...
int main() {
    ...
    cout << "=== TRIAD Trusted Dealer (Riboflavin) ===" << endl;
    cout << "  BATCH_SIZE=" << BATCH_SIZE << " NUM_PARTIES=" << NUM_PARTIES << endl;
    ...
    cout << "Keys saved (t=" << elapsed() << "s)" << endl;

    // 3. Threshold EvalMult
    ...
    for (auto& f : files) {
        ...
        total += mb;
        cout << "  " << f << ": " << fixed << setprecision(2) << mb << " MB" << endl;
    }
